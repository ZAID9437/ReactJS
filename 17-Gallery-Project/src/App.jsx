...
const App = () => {
  ...
  return (
    <div className='bg-black overflow-auto p-4 h-screen text-white'>
      {/* <button 
        onClick={getData}
        className='bg-green-600 active:scale-95 m-4 px-5 py-2 rounded textwhite'>
        Get Data
      </button> */}
      ...
      <div className='flex justify-center items-center gap-6 p-4'>
        <button style={{ opacity:index == 1 ? 0.5 : 1 }} className='bg-amber-400 text-sm cursor-pointer active:scale-95 text-black rounded px-4 py-2 font-semibold ' onClick={()=>{
        if(index > 1)
        {
          setIndex(index-1)
          setUserData([])
